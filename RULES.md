...
- **PLY-CHR-01 — Icon-only chrome.** Playback controls are icon-only with an accessible name, a `48×48dp` touch target on mobile, and a named token for every size (`PlayerChromeTokens`). Mobile transport and setting buttons carry no visible text; title, timecodes, and value chips remain.
- **PLY-CHR-02 — Single subtitle lift.** One lift value drives both engines: Media3 through the subtitle view and MPV through `sub-margin-y`, applied even when embedded ASS authoring is preserved. Subtitles never sit under the chrome, cutouts, or gesture zones.
- **PLY-CHR-03 — Lock layer.** Mobile may lock the chrome to prevent accidental touches. Lock hides the controls, keeps a visible unlock control, blocks only video-surface touches, and is unwound by Back like any other layer. TV has no lock.
- **PLY-CHR-04 — Gestures are accelerators.** Double-tap seek, drag-to-scrub, and vertical brightness/volume gestures must repeat through visible, screen-reader reachable controls: the rails, the ±10s buttons, and the scrubber. Gestures stay out of system gesture insets and are disabled while locked and in PiP.
- **PLY-CHR-05 — TV focus label.** The TV control row is icon-only at rest and reveals the focused control's label beside the row in the focused-content role. Focus treatment, geometry, and motion remain `TV-FOC-02`, `TV-LAY-01`, and `TV-MOT-01`.
- **PLY-CHR-06 — Brightness and volume.** Brightness is window- and session-scoped and is never a durable setting. Volume uses the system media stream with a player-volume fallback. Both expose range semantics and remain reachable without gestures.
