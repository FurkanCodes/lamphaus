...
| M9 | account-bound sync completion: add-on Edge Function client wiring (+ seed empty accounts, deletions converge at session start) · settings follow the account (jsonb payload LWW, sign-out wipe) · library/progress channels follow profile creation/removal | ✅ `685c17d`→`08b9df4` on `feature/account-bound-sync` · no DB changes (tables/RLS/publication from M1b suffice) · two-device E2E checklist pending |
